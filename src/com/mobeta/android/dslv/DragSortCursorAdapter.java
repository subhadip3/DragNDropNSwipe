...
/**
 * A subclass of {@link android.widget.CursorAdapter} that provides reordering of the elements in the Cursor based on
 * completed drag-sort operations. The reordering is a simple mapping of list positions into Cursor positions (the
 * Cursor is unchanged). To persist changes made by drag-sorts, one can retrieve the mapping with the
 * {@link #getCursorPositions()} method, which returns the reordered list of Cursor positions.
 *
 * An instance of this class is passed to {@link DragSortListView#setAdapter(ListAdapter)} and, since this class
 * implements the {@link DragSortListView.DragSortListener} interface, it is automatically set as the DragSortListener
 * for the DragSortListView instance.
 */
public abstract class DragSortCursorAdapter extends CursorAdapter implements DragSortListView.DragSortListener
{
	...
	/**
	 * On drop, this updates the mapping between Cursor positions and ListView positions. The Cursor is unchanged.
	 * Retrieve the current mapping with {@link getCursorPositions()}.
	 *
	...
	/**
	 * On remove, this updates the mapping between Cursor positions and ListView positions. The Cursor is unchanged.
	 * Retrieve the current mapping with {@link getCursorPositions()}.
	 *
	...
	/**
	 * Get the Cursor position mapped to by the provided list position (given all previously handled drag-sort
	 * operations).
	 *
	 * @param position
	 *            List position
	...
	/**
	 * Get the list position mapped to by the provided Cursor position. If the provided Cursor position has been removed
	 * by a drag-sort, this returns {@link #REMOVED}.
	 *
