 ...
 *
 * An instance of this class is meant to be passed to the methods {@link DragSortListView#setTouchListener()} and
 * {@link DragSortListView#setFloatViewManager()} of your {@link DragSortListView} instance.
 */
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener
{
	...
	/**
	 * Calls {@link #DragSortController(DragSortListView, int)} with a 0 drag handle id, FLING_RIGHT_REMOVE remove mode,
	 * and ON_DOWN drag init. By default, sorting is enabled, and removal is disabled.
	 *
	 * @param dslv
	 *            The DSLV instance
	 */
	/*public DragSortController(int x,int y)
	{
		global=x;
		super()
	}*/
	...
	/**
	 * Enable/Disable list item sorting. Disabling is useful if only item removal is desired. Prevents drags in the
	 * vertical direction.
	 *
	 * @param enabled
	...
	/**
	 * One of {@link CLICK_REMOVE}, {@link FLING_RIGHT_REMOVE}, {@link FLING_LEFT_REMOVE}, {@link SLIDE_RIGHT_REMOVE},
	 * or {@link SLIDE_LEFT_REMOVE}.
	 */
	...
	/**
	 * Sets flags to restrict certain motions of the floating View based on DragSortController settings (such as remove
	...
	public boolean onTouch(View v, final MotionEvent ev)
	{
		...
		/*
		 * if(event.getAction() == MotionEvent.ACTION_DOWN) handler.postDelayed(mLongPressed, 1000);
			...
			case MotionEvent.ACTION_DOWN:
				...
				if (mHitPos != MISS && mDragInitMode == ON_DOWN)
				{
					...
					if(bool==1)
					{
					  ...
					  Log.d("time", ""+estimatedTime);
					  Log.d(""+ev.getX(), ""+ev.getY());
					  x1=ev.getX();
					  mDslv.x1=x1;
	...
	/**
	 * Get the position to start dragging based on the ACTION_DOWN MotionEvent. This function simply calls
	 * {@link #dragHandleHitPosition(MotionEvent)}. Override to change drag handle behavior; this function is called
	...
	/**
	 * Checks for the touch of an item's drag handle (specified by {@link #setDragHandleId(int)}), and returns that
	 * item's position if a drag handle touch was detected.
	 *
	 * @param ev
	 *            The ACTION_DOWN MotionEvent.
	 * 
	...
	public void onLongPress(MotionEvent ev)
	{
		...
		/*
		 * if (mRemoveEnabled && mRemoveMode == CLICK_REMOVE) { mClickRemoveHitPos = viewIdHitPosition(ev,
		 * mClickRemoveId); } mHitPos = startDragPosition(ev); if (mHitPos != MISS && mDragInitMode == ON_LONG_PRESS) {
		 * startDrag(mHitPos, (int) ev.getX() - mItemX, (int) ev.getY() - mItemY); } mIsRemoving = false; mCanDrag =
		 * true; mPositionX = 0; mFlingHitPos = startFlingPosition(ev);
		 */
